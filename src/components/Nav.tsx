  ...
  return (
    <>
      ...
      <section className="page-title" aria-label="Titre de la page">
        <h1>{activeView === 'map' ? 'Mon carnet de voyages' : 'Explorer - Suggestions IA'}</h1>
        <p>{activeView === 'map' ? `${totalDestinations} destination${totalDestinations > 1 ? 's' : ''} notée${totalDestinations > 1 ? 's' : ''}` : 'Placeholder IA, bientot connecte a ton classement'}</p>
      </section>
    </>
  )
}
