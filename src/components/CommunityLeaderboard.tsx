...
// 0 = toutes. Défaut Top 10 : un classement court et lisible, pas un annuaire.
...
const TIER_HEADLINE: Record<Tier, string> = {
  ...
  B: t('Generally appreciated', 'Globalement appréciées'),
  C: t('The crowd is lukewarm', 'Le peuple est tiède'),
  D: t('The crowd says skip', 'Le peuple dit passe ton tour'),
}

// Les tiers bas démarrent repliés, comme dans la liste perso.
...
export function CommunityLeaderboard({
  ...
  // Filtres appliqués côté client sur les rows déjà triées par note desc.
  // Une recherche active ignore le Top N (on veut TOUS les résultats du terme).
  ...
  // "toi vs le peuple" : retrouver ma destination derrière chaque agrégat
  ...
  return (
    <section className="community-leaderboard page-mode-fade" aria-label={t('Global ranking', 'Classement global')}>
      <div className="community-leaderboard-intro">
        <p>
          {t(
            'Anonymous average of everyone using Outpost. A destination appears once 3 travelers have rated it.',
            'Moyenne anonyme de tous les utilisateurs d’Outpost. Une destination apparaît dès que 3 voyageurs l’ont notée.'
          )}
        </p>
        <input
          type="search"
      ...
      {!isEmpty && (
        <div className="community-leaderboard-filters">
          <SegmentedControl
            ...
            ariaLabel={t('Number of destinations shown', 'Nombre de destinations affichées')}
            role="radiogroup"
            size="sm"
            layout="scrollable"
            tone="tinted"
          ...
          />
          <SegmentedControl
            className="tier-list-filters community-filter-min"
            ariaLabel={t('Minimum number of ratings', "Nombre minimum d'avis")}
            role="radiogroup"
            size="sm"
            layout="scrollable"
            tone="tinted"
            ...
            options={[
              { value: '3', label: t('3+ ratings', '3+ avis') },
              { value: '5', label: t('5+ ratings', '5+ avis') },
              { value: '10', label: t('10+ ratings', '10+ avis') },
            ]}
            onChange={value => setMinRatings(Number(value) as MinRatings)}
          />
        </div>
      )}
      ...
      {isEmpty && !error && (
        <div className="empty-friend-carnet-card community-leaderboard-empty" role="status">
          <h3>{t('The crowd has not spoken yet', 'Le peuple n’a pas encore parlé')}</h3>
          <p>
            {t(
              'The community rating appears as soon as 3 travelers rate the same destination.',
              'La note du peuple apparaît dès que 3 voyageurs ont noté une même destination.'
            )}
          </p>
          {teaserCount !== null && teaserCount > 0 && (
            <p className="community-leaderboard-teaser">
              ...
              {t(
                ...
                teaserCount > 1 ? 'destinations sont à un avis du seuil' : 'destination est à un avis du seuil'
              )}
            </p>
          )}
        </div>
      ...
      {!loading && rows.length === 0 && search.trim() && !error && (
        <p className="community-leaderboard-status">{t('No destination found', 'Aucune destination trouvée')}</p>
      )}
      ...
      {TIER_ORDER.map(tier => {
        ...
        return (
          <article key={tier} className={`tier-list-row tier-list-row-${tier.toLowerCase()}`}>
            <header onClick={() => setCollapsed(prev => ({ ...prev, [tier]: !prev[tier] }))}>
                      ...
                      <>
                        ...
                        <div className="community-row-meta">
                          {row.topTags.slice(0, 2).map(tag => (
                            <span key={tag} className="community-row-tag">{tag}</span>
                          ))}
                          {row.topTags.length > 2 && (
                            <span className="community-row-tag" title={row.topTags.slice(2).join(' · ')}>
                              +{row.topTags.length - 2}
                            </span>
                          )}
                        </div>
                        <div className="community-row-scores">
                          {myTier && (
